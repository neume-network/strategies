...
const logs = [
  ...
  // NOTE: This is the log of a soundxyz artist that isn't appearing on any
  // static list of contracts.
  {
...
test("if event logs are appropriately migrated to web3subgraph compatible NFT IDs", (t) => {
  const contractsPath = resolve(__dirname, "./contracts");
  const contracts = JSON.parse(fs.readFileSync(contractsPath));
  const { write } = onLine(JSON.stringify(logs), contracts);
  const expected = [
    {
      address: "0xabefbc9fd2f806065b4f3c237d4b59d9a97bcac7",
      tokenId: "7437",
